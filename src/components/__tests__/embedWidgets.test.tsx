...
describe("MusicEmbedWidget", () => {
  it.each([
    ...
    "renders a %s player for a valid share url",
    (provider, shareUrl, expectedSrc) => {
      render(
        <MusicEmbedWidget
          title="Player"
          provider={provider}
      ...
      expect(
        screen.getByRole("link", { name: "Open full player" }),
      ).toBeInTheDocument();
    },
  );
...
describe("SpotifyWidget", () => {
  ...
  it("shows a connect button when Spotify is not linked", () => {
    ...
    expect(
      screen.getByText(/Connect Spotify to show the player here/i),
    ).toBeInTheDocument();
    expect(
      screen
        .getByRole("button", { name: /Connect Spotify/i })
        .querySelector("img"),
    ).toBeTruthy();
  });
  ...
  it("ignores recently played entries missing track details", async () => {
    ...
    expect(
      await screen.findByText(/Nothing played recently/i),
    ).toBeInTheDocument();
...
describe("AppleMusicWidget", () => {
  ...
  it("updates the saved Apple Music link list when a new url is added", async () => {
    ...
    fireEvent.change(
      ...
      {
        target: {
          value: "https://music.apple.com/us/album/midnights/1625498918",
        },
      },
    );
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTitle("Apple Music Player player")).toHaveAttribute(
        ...
        expect.stringContaining(
          "https://embed.music.apple.com/us/album/midnights/1625498918",
        ),
      );
    });
  });
